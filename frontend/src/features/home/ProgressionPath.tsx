...
const Step = ({
  ...
  <div
    className={`relative flex-1 group transition-all duration-700 ease-in-out hover:flex-[2.5] overflow-hidden min-h-[400px] md:min-h-[500px] first:rounded-l-3xl last:rounded-r-3xl animate-fade-in-up ${delay}`}
  >
    <img
      src={
        img.startsWith('/') ? `${import.meta.env.BASE_URL.replace(/\/$/, '')}${img}` : img
      }
      alt={title}
    ...
    <div
      className={`absolute ${bottom} left-1/2 -translate-x-1/2 w-[90%] transition-all duration-500 group-hover:-translate-y-2`}
    >
      <div className="text-accent-blue text-[8px] font-bold tracking-[0.3em] mb-1 text-center opacity-60 group-hover:opacity-100">
        {level}
      </div>
      <h4 className="text-sm md:text-xl font-bold mb-3 text-center whitespace-nowrap leading-tight">
        {title}
      </h4>
      ...
      <div className="glass p-4 rounded-xl opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-500 delay-75">
        <p className="text-white/70 text-[10px] leading-relaxed text-center font-light">
          {desc}
        </p>
      </div>
    </div>
        ...
        <div className="text-center mb-16 animate-fade-in-up">
          ...
          <h3 className="text-4xl md:text-5xl font-bold leading-tight">
            ...
            <span className="text-accent-blue italic">
              {t('progression.title_highlight')}
            </span>
          </h3>
        </div>

        <div className="flex flex-col lg:flex-row w-full h-auto gap-1 lg:gap-0 border border-border-light rounded-3xl overflow-hidden shadow-2xl">
          <Step
            level="Level 01"
